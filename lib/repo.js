...
Repo.prototype.auditPr = function( options, callback ) {
	...
	this.fetchPr( options.pr, function( error ) {
		...
		repo.getPrCommitRange( options.pr, function( error, commitRange ) {
			if ( error ) {
				return callback( error );
			}
...
Repo.prototype.getPrCommitRange = function( pr, callback ) {
	this.getPrCommits( pr, function( error, commits ) {
		if ( error ) {
			return callback( error );
		}

		// The commit range must start at the parent of the PR's first commit
		// because the base of a range is not inclusive
		var first = commits[ 0 ].sha + "^",
			last = commits.pop().sha,
			range = first + ".." + last;
...
Repo.prototype.getPrCommits = function( pr, callback ) {
	github.requestAll({
		path: "/repos/" + config.owner + "/" + this.name + "/pulls/" + pr + "/commits"
